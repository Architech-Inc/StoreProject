function LoadScripts(param1, param2, param3) {
    App.LoadScript(CONTENT_PATH + 'scripts/header.js', () => {
        alert(CONTENT_PATH)
        App.LoadScript(CONTENT_PATH + 'scripts/index.js', () => {

    /*App.LoadScript('./vendors/tailwindcss/tailwindcss.js', () => {
        App.LoadScript('./te-starter/js/tw-elements.umd.min.js', () => {

            //setEventHandlers()

            try {
                let frameName = '.revision-frame', bImgName = '.brand-img', wwName = '.welcome-wrapper', bfName = '.body-frame', tb = '.tab-contents', sk = '.sk-grid'
                var partialViewPortHeight = document.querySelector(frameName + ' ' + bImgName).getBoundingClientRect().height + parseFloat(getComputedStyle(document.querySelector(frameName + ' ' + bImgName)).marginBlock) * 2 + document.querySelector(frameName + ' ' + wwName).getBoundingClientRect().height + parseFloat(getComputedStyle(document.querySelector(frameName + ' ' + wwName)).marginBottom)

                const bodyFrames = document.querySelector(frameName + ' ' + bfName)
                //console.log(bodyFrames)
                //bodyFrames.forEach(element => {
                bodyFrames.setAttribute('style', `height: ${window.innerHeight - partialViewPortHeight}px; min-height: ${window.innerHeight - partialViewPortHeight}px; max-height: ${window.innerHeight - partialViewPortHeight}px;`)
                //});

                var tabContentHeight = parseInt(window.getComputedStyle(document.querySelector(tb)).getPropertyValue('height'))
                let height = tabContentHeight - 78

                const skGrids = document.querySelectorAll(frameName + ' ' + bfName + ' ' + tb + ' ' + sk)
                console.log(skGrids)
                skGrids.forEach(element => {
                    element.setAttribute('style', `height: ${height}px; min-height: ${height}px; max-height: ${height}px; overflow-y: auto;`)
                });

                //console.log(tabContentHeight, partialViewPortHeight)
            }
            catch (x) {
                console.log(x)
            }*/

            App.LoadScript(CONTENT_PATH + 'scripts/dashboard.js', () => {
                alert()
                if (param1) {
                    let callback = window[param1]
                    if (param3) {
                        if (!callback || callback == undefined || callback == null) return
                        callback(param2, param3);
                        console.log('Called on param3')
                        return
                    }
                    if (param2) {
                        if (!callback || callback == undefined || callback == null) return
                        callback(param2);
                        console.log('Called on param2')
                        return
                    }
                    callback()
                    console.log('Called on no param')
                }
